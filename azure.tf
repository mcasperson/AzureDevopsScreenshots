...
resource "azurerm_virtual_machine_extension" "test" {
  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
      "commandToExecute": "powershell.exe -Command \"Set-ExecutionPolicy Bypass -Scope Process -Force; Set-ExecutionPolicy Bypass -Scope Process -Force; iex ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1')); choco install -y git; cd c:\\; & 'C:\\Program Files\\Git\\bin\\git' clone https://github.com/OctopusDeploy/OctopusGuides.git; cd OctopusGuides; & 'C:\\Program Files\\Git\\bin\\git' pull; ./install.ps1 -Scripts azuredevops.pp,utilities.pp,azurewait.pp; & 'C:\\Program Files\\OpenJDK\\jdk-13\\bin\\java' --enable-preview '-Dwebdriver.gecko.driver=C:\\tools\\geckodriver.exe' '-DslackHookUrl=#{SlackWebHook}' '-DslackStepHandlerEnabled=true' -jar c:\\tools\\webdrivertraining-1.0-SNAPSHOT.jar --tags '(@login or @install-extensions)' features\\azuredevops\\azuredevops-aspnet-project.feature\""
    }
  PROTECTED_SETTINGS
}
